...
class ServerTest(testtools.TestCase):
    ...
    def test_server_object_no_logical_drives(self, get_all_details_mock):
        ...
        self.assertEqual('MSCC SmartRAID 3154-8e in Slot 2085', controller.id)
        ...
        physical_drives_expected = ['CN1:1:1', 'CN1:1:11', 'CN1:1:12',
                                    'CN1:1:2', 'CN1:1:3', 'CN1:1:4',
                                    'CN1:1:5']
        ...
        physical_drive = list(filter(lambda x: x.id == 'CN1:1:11',
        ...
        self.assertEqual(constants.INTERFACE_TYPE_SATA,
                         physical_drive.interface_type)
        self.assertEqual(constants.DISK_TYPE_SSD,
    ...
    def test_server_object_one_logical_drive(self, get_all_details_mock):
        ...
        self.assertEqual(99, logical_drive.size_gb)
    ...
    def test_get_controller_by_id(self, get_all_details_mock):
        ...
        id = 'MSCC SmartRAID 3154-8e in Slot 2085'
...
class ControllerTest(testtools.TestCase):
    ...
    @mock.patch('os.path.exists')
    @mock.patch.object(processutils, 'execute')
    def test_execute_cmd_fails(self, processutils_mock, mock_path,
                               get_all_details_mock):
    ...
    def test_create_logical_drive_with_physical_disks(self, execute_mock,
        ...
        logical_drive_info = {'size_gb': 50,
                              ...
                              'is_boot_volume': 'true',
                              'controller': "'MSCC SmartRAID 3154-8e in "
                                            "Slot 2085'",
        ...
        execute_mock.assert_called_once_with("create",
                                             "type=logicaldrive",
                                             "drives=CN1:1:1,CN1:1:2,CN1:1:3",
                                             "raid=1",
    ...
    def test_create_logical_drive_max_size_gb(self, execute_mock,
        ...
        logical_drive_info = {'size_gb': 'MAX',
                              'raid_level': '1',
                              'controller': "'MSCC SmartRAID 3154-8e "
                                            "in Slot 2085'",
    ...
    def test_create_logical_drive_raid_level_mapping(self, execute_mock,
        ...
        execute_mock.assert_called_once_with(
            ...
            "drives=CN1:1:1,CN1:1:2,CN1:1:3,CN1:1:4,CN1:1:5,CN0:1:6",
    ...
    def test_get_physical_drive_by_id(self, get_all_details_mock):
        ...
        self.assertEqual(physical_drive,
                         controller.get_physical_drive_by_id('CN1:1:1'))

        physical_drive = list(filter(lambda x: x.id == 'CN1:1:3',
                                     controller.unassigned_physical_drives))[0]
        self.assertEqual(physical_drive,
                         controller.get_physical_drive_by_id('CN1:1:3'))
    ...
    def test_erase_devices_sanitize_not_supported(self, execute_mock,
        ...
        d = [x for x in server.controllers[0].unassigned_physical_drives]
        controller = server.controllers[0]
        value = ("Drive CN1:2:1: This operation is not supported in this "
                 "physical drive")
        execute_mock.return_value = value
        controller.erase_devices(d)
        calls = [mock.call('pd CN1:2:1', 'modify', 'erase',
                           'erasepattern=overwrite', 'unrestricted=off',
                           'forced'),
                 mock.call('pd CN1:2:1', 'modify', 'erase',
...
class LogicalDriveTest(testtools.TestCase):
    ...
    def test_get_logical_drive_dict(self, get_all_details_mock):
        ...
        self.assertEqual(99, ret['size_gb'])
        ...
        self.assertEqual('0216D8AE8A02F3004A0    DFDE',
                         ret['volume_name'])
    ...
    def test___init__bad_size_logical_drive(self, get_all_details_mock):
        ...
        msg = ("unknown size '558.9foo' for logical disk '1' of RAID array "
               "'A' in controller 'MSCC SmartRAID 3154-8e in Slot 2085'")
...
class ArrayTest(testtools.TestCase):
    ...
    @mock.patch('os.path.exists')
    @mock.patch.object(processutils, 'execute')
    def test_can_accomodate_okay(self, execute_mock, mock_path,
                                 get_all_details_mock):
    ...
    @mock.patch('os.path.exists')
    @mock.patch.object(processutils, 'execute')
    def test_can_accomodate_max_size_gb_okay(self, execute_mock, mock_path,
                                             get_all_details_mock):
        current_config = raid_constants.HPSSA_TWO_DRIVES_100GB_RAID5_50GB_RAID1
        get_all_details_mock.return_value = current_config
        execute_mock.return_value = (
    ...
    @mock.patch('os.path.exists')
    @mock.patch.object(processutils, 'execute')
    def test_can_accomodate_not_enough_space(self, execute_mock, mock_path,
                                             get_all_details_mock):
        current_config = raid_constants.HPSSA_TWO_DRIVES_100GB_RAID5_50GB_RAID1
        get_all_details_mock.return_value = current_config
        execute_mock.return_value = (
    ...
    @mock.patch('os.path.exists')
    @mock.patch.object(processutils, 'execute')
    def test_can_accomodate_invalid_raid_level(self, execute_mock, mock_path,
                                               get_all_details_mock):
        current_config = raid_constants.HPSSA_TWO_DRIVES_100GB_RAID5_50GB_RAID1
        get_all_details_mock.return_value = current_config
    ...
    def test_can_accomodate_map_raid_level(self, execute_mock, path_mock,
        ...
        execute_mock.assert_called_once_with(
            "ssacli", "controller", "slot=2085", "array", mock.ANY, "create",
...
class PhysicalDriveTest(testtools.TestCase):
    ...
    def test___init__bad_size_logical_drive(self, get_all_details_mock):
        ...
        ex = self.assertRaises(exception.HPSSAOperationError,
                               objects.Server)
        msg = ("unknown size '500foo' for physical disk 'CN1:1:1' of "
    ...
    def test___init__physical_disk_ssd(self, get_all_details_mock):
        ...
        self.assertEqual(480, ret_sas['size_gb'])
        self.assertEqual('MSCC SmartRAID 3154-8e in Slot 2085',
                         ret_sas['controller'])
        self.assertEqual('CN1:2:14', ret_sas['id'])
        self.assertEqual('ssd', ret_sas['disk_type'])
        self.assertEqual('sas', ret_sas['interface_type'])
        self.assertEqual('ATA     MK000480GWEZH', ret_sas['model'])
        self.assertEqual('HPGB', ret_sas['firmware'])
        self.assertEqual('ready', ret_sas['status'])

        self.assertEqual('CN1:2:15', ret_sata['id'])
    ...
    def test_get_physical_drive_dict_part_of_array(self, get_all_details_mock):
        ...
        self.assertEqual('MSCC SmartRAID 3154-8e in Slot 2085',
                         ret['controller'])
        self.assertEqual('CN1:1:2', ret['id'])
        self.assertEqual('ssd', ret['disk_type'])
        self.assertEqual('sata', ret['interface_type'])
        self.assertEqual('ATA     MK000500GWEZH', ret['model'])
        self.assertEqual('HPGB', ret['firmware'])
    ...
    def test_get_physical_drive_dict_unassigned(self, get_all_details_mock):
        ...
        self.assertEqual('MSCC SmartRAID 3154-8e in Slot 2085',
                         ret['controller'])
        self.assertEqual(500, ret['size_gb'])
        self.assertEqual('CN1:1:3', ret['id'])
        self.assertEqual('ssd', ret['disk_type'])
        self.assertEqual('sata', ret['interface_type'])
        self.assertEqual('ATA     MK000500GWEZH', ret['model'])
        self.assertEqual('HPGB', ret['firmware'])
